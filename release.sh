...
SVN_REPO=https://plugins.svn.wordpress.org/public-media-platform

# which files go into the release?
GLOBIGNORE=*
WHITELIST=(LICENSE plugin.php README.txt assets/* inc/* templates/* vendor/*)
BLACKLIST=(vendor/pmpsdk.phar vendor/guzzle/guzzle/docs/* vendor/guzzle/guzzle/tests/*)
...
# (1) build vendoring
echo " - vendoring non-dev dependencies"
OUT=`make install-release 2>&1`
if [[ $? -ne 0 ]]; then echo "$OUT" && exit 1; fi
